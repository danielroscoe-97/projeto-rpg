"use client";

import { useEffect, useRef } from "react";
import Link from "next/link";
import { useTranslations } from "next-intl";
import { trackEvent } from "@/lib/analytics/track";
import { saveGuestCombatSnapshot, useGuestCombatStore } from "@/lib/stores/guest-combat-store";

const FOCUSABLE = 'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';

interface GuestExpiryModalProps {
  onReset: () => void;
}

export function GuestExpiryModal({ onReset }: GuestExpiryModalProps) {
  const t = useTranslations("guest");
  const ctaRef = useRef<HTMLAnchorElement>(null);
  const modalRef = useRef<HTMLDivElement>(null);
  const hasTrackedExpiry = useRef(false);

  // Track expiry event once
  useEffect(() => {
    if (!hasTrackedExpiry.current) {
      hasTrackedExpiry.current = true;
      trackEvent("guest:session_expired");
    }
  }, []);

  // Focus the primary CTA on mount
  useEffect(() => {
    ctaRef.current?.focus();
  }, []);

  // Tab cycling focus trap (no ESC close — blocking modal)
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        // Block ESC — this is a non-dismissable modal
        e.preventDefault();
        return;
      }
      if (e.key === "Tab" && modalRef.current) {
        const focusable = Array.from(
          modalRef.current.querySelectorAll<HTMLElement>(FOCUSABLE)
        );
        if (focusable.length === 0) return;
        const first = focusable[0];
        const last = focusable[focusable.length - 1];
        if (e.shiftKey) {
          if (document.activeElement === first) {
            e.preventDefault();
            last.focus();
          }
        } else {
          if (document.activeElement === last) {
            e.preventDefault();
            first.focus();
          }
        }
      }
    };
    document.addEventListener("keydown", handleKey);
    return () => document.removeEventListener("keydown", handleKey);
  }, []);

  const handleSignupClick = () => {
    trackEvent("guest:expired_cta_signup");
    // Save combat snapshot for post-signup migration
    const state = useGuestCombatStore.getState();
    saveGuestCombatSnapshot({
      combatants: state.combatants,
      currentTurnIndex: state.currentTurnIndex,
      roundNumber: state.roundNumber,
    });
  };

  const handleReset = () => {
    trackEvent("guest:expired_cta_reset");
    onReset();
  };

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4"
      role="dialog"
      aria-modal="true"
      aria-labelledby="expiry-modal-title"
    >
      {/* Backdrop — non-clickable */}
      <div
        className="absolute inset-0 bg-black/70 backdrop-blur-sm"
        aria-hidden="true"
      />

      {/* Modal */}
      <div ref={modalRef} className="relative bg-card border border-border rounded-xl shadow-2xl max-w-md w-full p-6 space-y-5">
        {/* No close button — blocking modal */}

        {/* Icon */}
        <div className="text-3xl text-center">&#x23F0;</div>

        {/* Title */}
        <div className="text-center space-y-2">
          <h2
            id="expiry-modal-title"
            className="font-display text-xl text-foreground"
          >
            {t("expired_title")}
          </h2>
          <p className="text-muted-foreground text-sm leading-relaxed">
            {t("expired_body")}
          </p>
        </div>

        {/* Actions */}
        <div className="flex flex-col gap-3 pt-1">
          <Link
            ref={ctaRef}
            href="/auth/sign-up?from=guest-expiry"
            onClick={handleSignupClick}
            className="relative overflow-hidden w-full text-center px-6 py-3 bg-gold text-surface-primary font-semibold rounded-lg hover:shadow-gold-glow hover:-translate-y-[1px] transition-all duration-[250ms] min-h-[48px] flex items-center justify-center btn-shimmer"
          >
            {t("expired_cta_signup")}
          </Link>
          <button
            type="button"
            onClick={handleReset}
            className="w-full text-center px-6 py-3 bg-white/[0.06] text-muted-foreground rounded-lg hover:bg-white/[0.1] transition-all duration-[250ms] min-h-[48px] text-sm"
          >
            {t("expired_cta_reset")}
          </button>
        </div>
      </div>
    </div>
  );
}
