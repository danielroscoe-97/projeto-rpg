-- 072_reconcile_force_delete.sql
-- Add p_force_delete_ids parameter to reconcile_combat_state RPC.
-- Allows DM to explicitly delete combatants even in merge_newer mode
-- (where player combatants are normally preserved).
...
CREATE OR REPLACE FUNCTION reconcile_combat_state(
  ...
  p_is_active BOOLEAN,
  p_merge_mode TEXT DEFAULT 'overwrite',
  p_force_delete_ids UUID[] DEFAULT ARRAY[]::UUID[]
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  local_ids UUID[];
  db_combatant RECORD;
  merge_result JSONB := '{"merged": 0, "inserted": 0, "deleted": 0, "force_deleted": 0}'::JSONB;
  v_count INT := 0;
...
BEGIN
  ...
  -- Collect local IDs for deletion step
  ...
  LOOP
    IF p_merge_mode = 'merge_newer' THEN
      ...
      IF FOUND AND db_combatant.is_player THEN
        -- Player combatant: keep DB HP values
      ...
      ELSE
        -- Monster/NPC or new combatant: full overwrite
        INSERT INTO combatants (
          id, encounter_id, name, current_hp, max_hp, temp_hp, ac, spell_save_dc,
          initiative, initiative_order, conditions, is_defeated, is_player, is_hidden,
          monster_id, display_name, monster_group_id, group_order, dm_notes,
  ...
  -- Step 2: Delete combatants removed during offline
  ...
  -- Step 2b: Force-delete — DM explicitly removed these combatants (even players)
  IF array_length(p_force_delete_ids, 1) > 0 THEN
    DELETE FROM combatants
    WHERE encounter_id = p_encounter_id
      AND id = ANY(p_force_delete_ids);
    GET DIAGNOSTICS v_count = ROW_COUNT;
    merge_result := jsonb_set(merge_result, '{force_deleted}', to_jsonb(v_count));
  END IF;

  -- Step 3: Update encounter state
  UPDATE encounters
...
GRANT EXECUTE ON FUNCTION reconcile_combat_state TO authenticated;
